 ...
 */
abstract class Pngx__Process__Queue extends Pngx__Process__Handler {
	...
	/**
	 * Pngx__Process__Queue constructor.
	 *
	 * @since 4.7.12
	 ...
	 *
	 * @see   Pngx__Process__Queue::save() to get the queue unique id.
	 *
	...
	public static function is_stuck( $queue_id ) {
		 ...
		 */
		$limit = (float) apply_filters( 'pngx_process_queue_time_limit', 300 );
		 ...
		 * @param Pngx__Data $queue_status
		 */
		return apply_filters( 'pngx_process_queue_is_stuck', $is_stuck, $queue_id, $queue_status );
	}
	 ...
	 * @param string $queue_id
	 *
	 * @return Pngx__Data An object containing information about the queue.
	 *
	 * @see   Pngx__Process__Queue::save() to get the queue unique id.
	 */
	public static function get_status_of( $queue_id ) {
		...
		$data = [
			'identifier'  => $queue_id,
			'done'        => (int) Pngx__Utils__Array::get( $meta, 'done', 0 ),
			'total'       => (int) Pngx__Utils__Array::get( $meta, 'total', 0 ),
			'fragments'   => (int) Pngx__Utils__Array::get( $meta, 'fragments', 0 ),
			'last_update' => (int) Pngx__Utils__Array::get( $meta, 'last_update', false ),
		];

		return new Pngx__Data( $data, 0 );
	}
	...
	public static function delete_all_queues( $action ) {
		...
		$action = $wpdb->esc_like( 'pngx_queue_' . $action ) . '%';
	...
	public function update( $key, $data ) {
		 ...
		 * @param self $this
		 */
		$update_data = apply_filters( "pngx_process_queue_{$this->identifier}_update_data", $update_data, $this );
	...
	public function save() {
		 ...
		 * @param self $this
		 */
		$save_data = apply_filters( "pngx_process_queue_{$this->identifier}_save_data", $save_data, $this );
