...
/**
 * Class Pngx__Process__Post_Thumbnail_Setter

 *
 * Handles upload and setting of a post thumbnail in an async process.
 * Example usage:
 *
 *      $post_thumbnail_setter = new Pngx__Process__Post_Thumbnail_Setter();
 ...
 * @since 4.7.12
 */
class Pngx__Process__Post_Thumbnail_Setter extends Pngx__Process__Handler {
	/**
	...
	public function dispatch() {
		...
		do_action( 'pngx_log', 'debug', __CLASS__, $data );
	 ...
	 *
	 * The post thumbnail will be uploaded, if not uploaded already, using the `pngx_upload_image` function.
	 * This method is an alias of the publicly accessible `sync_handle` one.
	 *
	 * @since 4.7.12
	 *
	 * @see   pngx_upload_image()
	 * @see   Pngx__Process__Post_Thumbnail_Setter::sync_handle()
	 *
	...
	public function sync_handle( ?array $data_source = null ) {
		...
		if ( ! isset( $data_source['post_id'], $data_source['post_thumbnail'] ) ) {
			do_action( 'pngx_log', 'error', $this->identifier, [ 'data' => $data_source, ] );
		...
		$thumbnail_id = pngx_upload_image( $post_thumbnail );
