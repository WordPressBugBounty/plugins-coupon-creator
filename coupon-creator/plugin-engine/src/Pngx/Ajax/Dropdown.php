...
class Dropdown {
	...
	public function hook() {
		add_action( 'wp_ajax_pngx_dropdown', [ $this, 'route' ] );
		add_action( 'wp_ajax_nopriv_pngx_dropdown', [ $this, 'route' ] );
	}
	...
	public function search_terms( $search, $page, $args, $source ) {
		...
		if ( empty( $args['taxonomy'] ) ) {
			$this->error( esc_attr__( 'Cannot look for Terms without a taxonomy', 'plugin-engine' ) );
		}
	 ...
	 * It is like a Catch All on `wp_ajax_pngx_dropdown` and `wp_ajax_nopriv_pngx_dropdown`
	 *
	 * @since 4.0.0
	 *
	 * @return void
	...
	public function route() {
		...
		if ( empty( $data ) ) {
			$this->error( esc_attr__( 'Empty data set for this dropdown', 'plugin-engine' ) );
		} else {
			$this->success( $data );
		}
	}
	...
	public function __call( $name, $arguments ) {
		$message = __( 'The "%s" source is invalid and cannot be reached on "%s" instance.', 'plugin-engine' );
