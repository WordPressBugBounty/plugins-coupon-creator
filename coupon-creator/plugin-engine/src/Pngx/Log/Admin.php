...
class Pngx__Log__Admin {
	public function __construct() {
		add_action( 'wp_ajax_pngx_logging_controls', [ $this, 'listen' ] );
	...
	public function display_log() {
		...
		ob_start();
		include trailingslashit( Pngx__Main::instance()->plugin_path ) . 'src/admin-views/event-log.php';
		return ob_get_clean();
	}
	...
	public function listen() {
		 ...
		 */
		do_action( 'pngx_common_update_log_settings' );
		 ...
		 */
		do_action( 'pngx_common_updated_log_settings' );
	...
	protected function get_available_logs() {
		...
		if ( empty( $available_logs ) ) {
			return [ '' => _x( 'None currently available', 'log selector', 'pngx-common' ) ];
		}
	...
	protected function get_log_engines() {
		...
		if ( empty( $available_engines ) ) {
			return [ '' => _x( 'None currently available', 'log engines', 'pngx-common' ) ];
		}
	...
	public function serve_log_downloads() {
		if ( empty( $_GET['pngx-common-log'] ) || 'download' !== $_GET['pngx-common-log'] ) {
			return;
		}
		 ...
		 */
		$log_name = apply_filters( 'pngx_common_log_download_filename', $log_name );

		header( 'Content-Disposition: attachment; filename="pngx-log-' . $log_name . '"' );
