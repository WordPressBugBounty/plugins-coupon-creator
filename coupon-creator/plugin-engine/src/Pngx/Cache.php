...
class Pngx__Cache implements ArrayAccess {
	const SCHEDULED_EVENT_DELETE_TRANSIENT = 'pngx_schedule_transient_purge';
	...
	public static function setup() {
		wp_cache_add_non_persistent_groups( [ 'pngx-non-persistent' ] );
	}
	...
	public function set( $id, $value, $expiration = 0, $expiration_trigger = '' ) {
		 ...
		 */
		$expiration = apply_filters( 'pngx_cache_expiration', $expiration, $id, $value, $expiration_trigger, $key );

		if ( self::NON_PERSISTENT === $expiration ) {
			$group      = 'pngx-non-persistent';
			$expiration = 1;
	...
	public function get( $id, $expiration_trigger = '', $default = false, $expiration = 0, $args = [] ) {
		$group   = isset( $this->non_persistent_keys[ $id ] ) ? 'pngx-non-persistent' : 'pngx';
	...
	/**
	 * Purge all expired pngx_ transients.
	 *
	...
	public function delete_expired_transients() {
		if ( pngx_get_var( 'has_deleted_expired_transients', false ) ) {
			return;
		}
		...
		$sql = "
			...
			WHERE
				a.option_name LIKE '\_transient\_pngx\_%'
				AND a.option_name NOT LIKE '\_transient\_timeout\_pngx\_%'
		";
		 ...
		 */
		$sql = apply_filters( 'pngx_cache_delete_expired_transients_sql', $sql, $time );
		...
		pngx_set_var( 'has_deleted_expired_transients', true );
	}
	...
	public function flag_required_delete_transients( $value = true ) {
		pngx_set_var( 'should_delete_expired_transients', $value );
	}
	...
	public function maybe_delete_expired_transients() {
		if ( ! pngx_get_var( 'should_delete_expired_transients', false ) ) {
			return;
		}
	...
	public function get_id( $key, $expiration_trigger = '' ) {
		...
		foreach ( $triggers as $trigger ) {
			// Bail on empty trigger otherwise it creates a `pngx_last_` opt on the DB.
			if ( empty( $trigger ) ) {
				continue;
			}
	...
	public function get_last_occurrence( $action ) {
		...
		$cache_last_actions = pngx_get_var( $cache_var_name, [] );

		if ( isset( $cache_last_actions[ $action ] ) ) {
			return $cache_last_actions[ $action ];
		}

		$last_action = (float) get_option( 'pngx_last_' . $action, null );
		...
		pngx_set_var( $cache_var_name, $cache_last_actions );
	...
	public function set_last_occurrence( $action, $timestamp = 0 ) {
		...
		$updated = update_option( 'pngx_last_' . $action, (float) $timestamp );
	...
	public function warmup_post_caches( $post_ids, $update_post_meta_cache = false ) {
		 ...
		 */
		$limit = (int) apply_filters( 'pngx_cache_warmup_post_cache_limit', min( $limit, count( $post_ids ) ) );
	 ...
	 *
	 * @return string The cache name with the prefix(microtime when name group created).
	 */
	public function get_cache_prefix( $group ) {
		// Get cache key - uses cache key pngx_sessions_cache_prefix to invalidate when needed.
		$prefix = wp_cache_get( 'pngx_' . $group . '_cache_prefix', $group );

		if ( false === $prefix ) {
			$prefix = microtime();
			wp_cache_set( 'pngx_' . $group . '_cache_prefix', $prefix, $group );
		}

		return 'pngx_cache_' . $prefix . '_';
	}
	...
	public function invalidate_cache_group( $group ) {
		// Update the cache group with microtime invalidates the existing entries.
		wp_cache_set( 'pngx_' . $group . '_cache_prefix', microtime(), $group );
	}
}
