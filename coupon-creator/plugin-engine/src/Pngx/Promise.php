...
class Pngx__Promise extends Pngx__Process__Queue {
	...
	/**
	 * Pngx__Promise constructor.
	 *
	 * @param string|array|Pngx__Utils__Callback $callback   The callback that should run to perform the promise task.
	 ...
	 * @since 4.9.5
	 *
	 * @param callable|Pngx__Utils__Callback $resolved            The callback to call on success.
	 * @param callable|Pngx__Utils__Callback $rejected            The callback to call on failure.
	 ...
	 *                                                             callback.
	 *
	 * @return Pngx__Promise This promise.
	 *
	...
	public function error_handler( $code, $error_message ) {
		...
		$message .= "\n\t" . $error_message;
		pngx( 'pngx.logger' )->log( $message, Pngx__Log::ERROR, __CLASS__ );
	}
	 ...
	 *                                                   {
	 * @param callable|Pngx__Utils__Callback $callback  The callback this promise will
	 *                                                   call to perform the task.
	 * @param array                           $args      An array of arguments that will be passed to the callback.
	 * @param callable|Pngx__Utils__Callback $then      The callback this promise will
	 *                                                   call when complete.
	 ...
	 * @since 4.9.5
	 *
	 * @return Pngx__Process__Queue This object.
	 */
	 ...
	 * @since 4.9.5
	 *
	 * @param            callable|Pngx__Utils__Callback $callback      The callback to call.
	 ...
	 * Pngx__Utils__Callback class.
	 *
	 * @since 4.9.5
	 *
	 * @param string|array|Pngx__Utils__Callback $callback The callback to unpack.
	 *
	 * @return array|string A callable array of string.
	 */
	protected function unpack_callback( $callback ) {
		if ( $callback instanceof Pngx__Utils__Callback ) {
			$callback = [ pngx( $callback->get_slug() ), $callback->get_method() ];
		}
